...
if pluginConfig.enabled then
    ...
    local function SendMessage(type, source, message)
        ...
        if type == "dispatch" then
            SendNotification(source, "Dispatch", message, "info")
        elseif type == pluginConfig.emergencyCallType then
            SendNotification(source, type, message, "error")
        elseif type == pluginConfig.civilCallType then
            SendNotification(source, type, message, "warning")
        elseif type == pluginConfig.dotCallType then
            SendNotification(source, type, message, "warning")
        elseif type == "error" then
            SendNotification(source, "Error", message, "error")
        elseif type == "debug" and Config.debugMode then
            SendNotification(source, "Debug", message, "info")
        end
    end
        ...
        if shouldNotifyUnits() then
            ...
            for i = 0, GetNumPlayerIndices()-1 do
                ...
                if IsPlayerOnDuty(player) then
                    SendNotification(player, type, message, "info", message)
                else
    ...
    AddEventHandler("SonoranCAD::pushevents:UnitAttach", function(call, unit)
        ...
        if officerId ~= nil then
            ...
            SendNotification(officerId, "Dispatch", msg, "info", msg)
        ...
        if pluginConfig.enableCallerNotify and callerId ~= nil and call.dispatch.metaData.silentAlert == "false" then
            local callerMessage = pluginConfig.notifyMessage:gsub("{officer}", unit.data.name)
            SendNotification(callerId, "Dispatch", callerMessage, "info", callerMessage)
        else
        ...
        if call.idents ~= nil and type(noteContent) == 'string' then
            for _, ident in pairs(call.idents) do
                ...
                if officerId ~= nil then
                    ...
                    SendNotification(officerId, "Dispatch", message, "info", message)
                else
